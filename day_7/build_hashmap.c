// In this assignment, you will implement the Map class 
// using a hash map as the inderlying structure rather than a linked list. 
// The easiest approach to this assignment is to start with your implementation of Map 
// using a linked list and then adapt it from a single head value to an array of head values 
// that each point to an independent linked list. 
...
struct HashMapEntry* __HashMap_find(struct HashMap* self, char *key, int bucket)
{
    /* TODO: Add some code here */
  	...
  	for (curr = self->__heads[bucket]; curr != NULL; curr = curr->__next) {
