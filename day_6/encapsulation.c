// This assignment will refactor your implementation of your Python dict class 
// from a previous assignment following the principle of encapsulation. 
// We will rename member variables and functions to follow the Python convention 
// that "private" variables start with a double underscore (a.k.a. dunder) in C. 
...
int __Map_get(struct Map* self, char *key, int def)
{
    /* You need some code here */
  	struct MapEntry *curr = self->__head;
  	while (curr != NULL) {
