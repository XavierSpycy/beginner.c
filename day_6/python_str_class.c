// You will a set of supporting functions 
// using the following structure to implement a subset of the Python str class in C. 
...
// If as data is added, it exceeds the length of the buffer use realloc() to expand the buffer by 10. 
...
void pystr_appends(struct pystr* self, char *str) {
    /* Need a line or two of code here */
  	int i;
  	for (i = 0; str[i]; i++) pystr_append(self, str[i]);
}
